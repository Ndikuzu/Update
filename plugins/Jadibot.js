...
let handler = async (m, { conn, args, usedPrefix, command, isOwner, text }) => {
  //  if(!text) throw `/${command} 6282xxxxxx\nAwali dengan angka 62`
...
let user = global.db.data.users[m.sender]
    ...
    if (fs.existsSync(authFile)) {
                ...
                let authFiled = 'plugins/jadibot/'+m.sender
   // let isInits = !fs.existsSync(authFile)
    let { state, saveCreds} = await useMultiFileAuthState(authFiled)
  ...
  //  },
   ...
   // global.conn = conn; // Mengatur variabel conn sebagai global.conn
    conn.isInit = false
    let ev = conn.ev
     ...
     if (!conn.authState.creds.registered) {
        setTimeout(async () => {
          let codek = await conn.requestPairingCode(
            parseInt(await text.replace(/[^0-9]/g, ""))
          );
          codek = codek?.match(/.{1,4}/g)?.join("-") || codek;
          m.reply("code anda adalah: " + codek)
   ...
   const {
		...
		if (connection == 'connecting') m.reply('Trying To Connect..')
		if (connection == 'close') m.reply('⏱️ koneksi terputus & mencoba menyambung ulang...')
        if (connection == 'open') m.reply('✅ Tersambung')
...
let user = global.db.data.users[m.sender]
    ...
    let authFiles = 'plugins/jadibot/'+m.sender
  //  if (fs.existsSync(authFile)) {
                        if (global.conns instanceof Array) console.log()
else global.conns = []
               // let authFiles = 'database/jadibot/'+user_id
   // let isInits = !fs.existsSync(authFile)
    let { state, saveCreds} = await useMultiFileAuthState(authFiles)
    let msgRetryCounterCache = new NodeCache() 
